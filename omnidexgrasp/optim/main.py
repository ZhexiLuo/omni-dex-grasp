...
def process_single_task(
    ...
    # 🚀 Run optimization
    (optim_out / "debug").mkdir(parents=True, exist_ok=True)
    hoi_sync = HOI_Sync(str(optim_out), project_root=project_root)
...
def main(cfg: DictConfig) -> None:
    ...
    for task_dir in sorted(datasets_dir.iterdir()):
        ...
        if not (out_dir / "hand_params.pt").exists():
            ...
            continue
        try:
            process_single_task(task_dir.name, datasets_dir, output_dir, project_root)
        except Exception as e:
            logging.error(f"❌ {task_dir.name}: {e}")

    # 🧹 Cleanup
    torch.cuda.empty_cache()
    gc.collect()
    os._exit(0)  # nvdiffrast CUDA context cleanup
